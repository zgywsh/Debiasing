...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/train_outputs/gen_images/images_debias",
        # required=True
    )
    parser.add_argument(
        "--save_dir_ori",
        type=str,
        default="/train_outputs/gen_images/images_ori",
        # required=True
    )
    parser.add_argument(
        "--random_seed",
...
def gen_ori(rank, args):
    ...
    for i, prompt_i in enumerate(prompts_to_process):
        ...
        torch.save(
            images_tensor,
            f"/train_outputs/gen_tensors/step_0/{prompt_i}.pt",
        )
...
def gen_images(rank, args, step):
    ...
    pretrained_weights_path = (
        "/train_outputs/unet/unet_weights.pth"  # 修改为权重文件路径
    )
...
if __name__ == "__main__":
    ...
    from unet_loss_race import eval_images
    ...
    unet_w.requires_grad_(False)
    params_train = find_param("/index/race/race_index.txt")
    print(len(params_train))
    with open("/index/race/race_index.txt", "r") as file:
    ...
    for epoch in range(total_epochs):
        ...
        for i, prompt_i in enumerate(prompts_to_process):
            # print(prompt_i)
            images_ori_path = f"/train_outputs/gen_tensors/step_0/{prompt_i}.pt"
            ...
            targets = eval_images(images_tensor_w, images_ori_tensor, test=True)
            for j in tqdm(range(num_parts), desc="Processing", unit="part"):
                ...
                true_loss = eval_images(images, images_ori, iftargets=target)
        ...
        if epoch == 0:
            ...
            torch.save(
                unet_w.state_dict(),
                f"/train_outputs/unet/unet_weights.pth",
            )
            print("saving")
    unet_gen_images(-1)
